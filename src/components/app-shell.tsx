...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-background">
      <aside className="hidden sm:flex w-56 lg:w-64 shrink-0 flex-col bg-sidebar text-sidebar-foreground border-r border-sidebar-border">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="sm:hidden flex items-center gap-2 px-4 py-3 bg-sidebar text-sidebar-foreground border-b border-sidebar-border">
        ...
        <nav className="sm:hidden flex overflow-x-auto gap-1 px-2 py-2 bg-card border-b border-border">
          {nav.map((item) => {
            const active =
              item.to === "/" ? pathname === "/" : pathname.startsWith(item.to);
            return (
